...
# linked_list.head, linked_list.head.next 모두 '노드'가 들어감!!
# each nodes consist of item & next node
...
# create head attribute : head 자리에 노드가 들어감 & 시작 노드 = head
...
if __name__ == '__main__' : 
    ...
    # assign item values / put items on each nodes
    linked_list.head = Node(1)
    second = Node(2)
    third = Node(3)
    
    # connect nodes : '노드끼리' 연결해줘야 함
...
# Delete Implementation 1 << 이거 개중요!!
# position 말고 value값으로 직접 제거하는 법 찾아보기
...
class LinkedList :
    ...
    # Insert after a node : head는 건들지 않음
    ...
    def deleteNode(self,position) : 
        ...
        # head의 node만 바로 뒤의 node로 바꿔주면 끝
        if position == 0 :
            self.head = temp.next
            temp = None
            return
        ...
        # If the key is not present  : exception managing 2 cases 
        if temp is None : 
            return
